...
// from mat4
Vec3 transform(const Mat4 &m, Vec3 a) {
  float   x = m.v00 * a.x + m.v01 * a.y + m.v02 * a.z + m.v03;
  float   y = m.v10 * a.x + m.v11 * a.y + m.v12 * a.z + m.v13;
  float   z = m.v20 * a.x + m.v21 * a.y + m.v22 * a.z + m.v23;
  float div = m.v30 * a.x + m.v31 * a.y + m.v32 * a.z + m.v33;
...
void render(uint32_t time_ms) {
  ...
  Vec3 transformed_light = transform(inverse_object_transformation, light);
  ...
  for (uint32_t gi = 0; gi < link_object->gc; gi++) {
    ...
    for (uint32_t fi = 0; fi < g->fc; fi++) {
      ...
      Vec3 vertices[3] = {
        transform(transformation, link_object->v[f->v[0]]),
        transform(transformation, link_object->v[f->v[1]]),
        transform(transformation, link_object->v[f->v[2]])
      };
